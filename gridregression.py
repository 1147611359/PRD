...
class GridOption:
	def __init__(self, dataset_pathname, options):
		...
		if sys.platform != 'win32':
			self.svmtrain_pathname = os.path.join(dirname, '/home/shenwanxiang/libsvm/libsvm-3.21/PRD/svm-train')
			self.gnuplot_pathname = '/usr/bin/gnuplot'
		else:
			# example for windows
			self.svmtrain_pathname =  r"C:\libsvm-3.18\windows\svm-train.exe"
			# svmtrain_pathname = r'c:\Program Files\libsvm\windows\svm-train.exe'
			self.gnuplot_pathname = r"C:\gp501-win32-mingw\gnuplot\bin\gnuplot.exe"
		self.fold = 5
